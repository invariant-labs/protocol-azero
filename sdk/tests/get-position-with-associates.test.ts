import {
  ...
  describe('get-position-with-associates', async () => {
    ...
    it('position, pool and ticks match', async () => {
      ...
      const [position, pool, lowerTick, upperTick] = await invariant.getPositionWithAssociates(account.address, 0n)
    ...
    it('position does not exist', async () => {
        assertThrowsAsync(invariant.getPositionWithAssociates(account.address, 0n), InvariantError.PositionNotFound)
    })
  })
