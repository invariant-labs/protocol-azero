...
pub fn get_tick_at_sqrt_price(
    sqrt_price_decimal: SqrtPrice,
    tick_spacing: u16,
) -> TrackableResult<i32> {
    if sqrt_price_decimal.v > MAX_SQRT_PRICE || sqrt_price_decimal.v < MIN_SQRT_PRICE {
        return Err(err!("sqrt_price out of range"));
    }

    let sqrt_price_x32: u64 = sqrt_price_to_x32(sqrt_price_decimal);
...
mod tests {
    ...
    fn test_log2_x32() {
        ...
        // log2 of sqrt(1.0001^(-19_999)) - 1
        {
            ...
            sqrt_price_decimal = sqrt_price_decimal - SqrtPrice::new(1);
            let sqrt_price_x32 = sqrt_price_to_x32(sqrt_price_decimal);
            let (sign, value) = log2_iterative_approximation_x32(sqrt_price_x32);
            assert_eq!(sign, false);
