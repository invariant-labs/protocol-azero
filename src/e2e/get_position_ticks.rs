...
pub mod e2e_tests {
    ...
    async fn test_get_position_ticks(mut client: ink_e2e::Client<C, E>) -> E2EResult<()> {
        ...
        let result = get_position_ticks!(client, dex, address_of!(Alice), 0);
    ...
    async fn test_get_position_ticks_limit(mut client: ink_e2e::Client<C, E>) -> E2EResult<()> {
        ...
        for i in 1..=POSITION_TICK_LIMIT / 2 {
            let lower_tick = get_tick!(client, dex, pool_key, -(i as i32)).unwrap();
            let upper_tick = get_tick!(client, dex, pool_key, i as i32).unwrap();
    ...
    async fn test_get_position_ticks_with_offset(
        ...
        let result_1 = get_position_ticks!(client, dex, address_of!(Alice), 0);
        assert_eq!(result_1.len(), 4);

        let result_2 = get_position_ticks!(client, dex, address_of!(Alice), 1);
