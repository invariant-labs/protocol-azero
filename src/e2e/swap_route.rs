...
pub mod e2e_tests {
    ...
    use crate::{
        contracts::{entrypoints::InvariantEntrypoints, FeeTier, PoolKey, SwapHop},
    ...
    async fn swap_route(mut client: ink_e2e::Client<C, E>) -> E2EResult<()> {
        let (dex, token_x, token_y, token_z) = init_dex_and_3_tokens!(client);

        let alice = ink_e2e::alice();
        approve!(client, token_x, dex.account_id, u64::MAX as u128, alice).unwrap();
        approve!(client, token_y, dex.account_id, u64::MAX as u128, alice).unwrap();
        approve!(client, token_z, dex.account_id, u64::MAX as u128, alice).unwrap();
        ...
        let pool_key_1 = PoolKey::new(token_x.account_id, token_y.account_id, fee_tier).unwrap();
        let pool_key_2 = PoolKey::new(token_y.account_id, token_z.account_id, fee_tier).unwrap();
        ...
        let expected_token_amount = quote_route!(client, dex, amount_in, swaps.clone()).unwrap();
        ...
        let bob_amount_x = balance_of!(client, token_x, address_of!(Bob));
        let bob_amount_y = balance_of!(client, token_y, address_of!(Bob));
        let bob_amount_z = balance_of!(client, token_z, address_of!(Bob));

        assert_eq!(bob_amount_x, 0);
        assert_eq!(bob_amount_y, 0);
        assert_eq!(bob_amount_z, 986);

        let pool_1_after = get_pool!(
            client,
            dex,
            token_x.account_id,
            token_y.account_id,
        ...
        let alice_amount_x_before = balance_of!(client, token_x, address_of!(Alice));
        let alice_amount_y_before = balance_of!(client, token_y, address_of!(Alice));
        let alice_amount_z_before = balance_of!(client, token_z, address_of!(Alice));

        claim_fee!(client, dex, 0, alice).unwrap();
        claim_fee!(client, dex, 1, alice).unwrap();

        let alice_amount_x_after = balance_of!(client, token_x, address_of!(Alice));
        let alice_amount_y_after = balance_of!(client, token_y, address_of!(Alice));
        let alice_amount_z_after = balance_of!(client, token_z, address_of!(Alice));
