...
pub mod e2e_tests {
    ...
    async fn test_cross_both_side(mut client: ink_e2e::Client<C, E>) -> E2EResult<()> {
        ...
        mint!(client, token_x, address_of!(Alice), massive_x, alice).unwrap();
        mint!(client, token_y, address_of!(Alice), massive_y, alice).unwrap();
        approve!(client, token_x, dex.account_id, massive_x, alice).unwrap();
        approve!(client, token_y, dex.account_id, massive_y, alice).unwrap();
