...
macro_rules! create_pool {
    ($client:ident, $dex:ty, $dex_address:expr, $pool_key:expr, $init_sqrt_price:expr, $init_tick:expr, $caller:ident) => {{
        let message = build_message::<$dex>($dex_address.clone())
            .call(|contract| contract.create_pool($pool_key, $init_sqrt_price, $init_tick));
        let result = $client
            .call_dry_run(&$caller, &message, 0, None)
            .await
            .return_value();
